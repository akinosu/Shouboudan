...
<div class="container-fluid">
    ...
    <div class="flex-wrapper d-flex flex-wrap mt-5 mr-3 ml-3 justify-content-between">
        <div class="">
            <h3><a class="text-body" id="titel_pref"
                    href="{{route('pref_id.index', ['pref_id'=>$pref_id])}}">{{$pref_name->pref_name}}の消防団</a></h3>
        </div>
        <div class="">
    ...
    <div class="accordion mt-4" id="accordion-4">
        <div class="card">
            <div class="card-header" id="header-4a">
                <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#card-4a"
            ...
            <div id="card-4a" class="collapse" aria-labelledby="header-4a" data-parent="#accordion-4">
                <div class="card-body">
                    ...
                    <span class="btn"><a
                            href="{{ route('pref_id.index', ['city_id'=>$city->id,'pref_id'=>$city->pref_id]) }}"
                            title="{{ $city->city_name }}">{{ $city->city_name}}</a>( {{($city->posts->count())}}
                        )</span>
                    @endforeach
                </div>
            </div>
            ...
            <div class="card  mb-3">
                <div class="card-haeder p-3 w-100 d-flex flex-wrap">
                    <div class="d-flex flex-grow-1">
                        ...
                        @else
                        <img src="{{ asset('storage/profile_image/20200501_noimage.png') }}" class="rounded-circle"
                            width="50" height="50">
                        @endif

                        <div class="ml-2 d-flex flex-column flex-grow-1">
                            <p class="mb-0 text-nowrap"><a
                                    href="{{ route('pref_id.index', ['pref_id'=>$post->city->pref_id]) }}"
                                    class="mb-0">{{ $post->city->pref_name }}</a>
                                /<a href="{{ route('pref_id.index', ['city_id'=>$post->city->id,'pref_id'=>$post->city->pref_id]) }}"
                                    class="mb-0">{{ $post->city->city_name }}</a>のクチコミ</p>
                            <div class="d-flex flex-grow-1">
                                <a href="{{url('users/' .$post->user_id)}}" class="mb-0 text-nowrap">{{
                                    $post->user->name }}</a>
                    ...
                    <div class="ml-5 d-flex flex-column flex-grow-1">
                        <p class="mb-0 text-secondary text-nowrap mb-1">
                            <span class="title-label text-white bg-primary">居住期間</span>
                            @if ($post->end != null)
                            @if ($post->start != null)
                            {{$post->start}}年
                            <span>~</span>
                        ...
                        <p class="mb-0 text-secondary text-nowrap">
                            <span class="title-label text-white bg-primary">地域との関係</span>
                            @if ($post->immigrant === 0)
                            出身者
                            @else
                            移住者
                ...
                <div class="card-body">
                    ...
                    <h5><a class="card-title"
                            href="{{ route('pref_id.show' ,['post_id'=>$post->id, 'pref_id'=>$pref_id,'city_id'=>$city_id,'searchword'=> $searchword]) }}">{{
                            $post->subject }}</a></h5>
                ...
                <div class="card-footer py-1 d-flex justify-content-end bg-white flex-wrap">
                    ...
                    <div class="mr-3 d-flex align-items-center">
                        <a
                            href="{{ route('show_comment' ,['post_id'=>$post->id, 'pref_id'=>$pref_id, 'city_id'=>$city_id,'searchword'=> $searchword, 'comment'=>'#comment']) }}"><i
                                class="far fa-comment fa-fw fa-lg"></i></a>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <a class="far fa-heart fa-fw fa-lgtext-decoration-none"
                            href="{{ route('nice', ['post_id'=>$post->id]) }}" class="mb-0"></a>
                        @else
                        <a class="fas fa-heart fa-fw fa-lg text-decoration-none"
                            href="{{ route('unnice', ['post_id'=>$post->id]) }}" class="mb-0"></a>
                        @endif
